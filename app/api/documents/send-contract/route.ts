 ...
 * Generate and email a contract PDF
 *
 * Body:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.contractId) {
      ...
      propertyAddress = `${contract.property.address}, ${contract.property.city}, ${contract.property.state} ${contract.property.zip}`;

      // Check if this is an assignment contract
      if (contract.assignment) {
        if (!contract.assignment.buyer.email && !body.to) {
          return NextResponse.json(
            { error: 'Buyer has no email address. Please provide a "to" email.' },
            { status: 400 }
          );
        }

        recipientEmail = body.to || contract.assignment.buyer.email!;
        recipientName = contract.assignment.buyer.name;
        ...
        // Regular purchase contract - send to seller
        recipientEmail = body.to;
        recipientName = contract.property.ownerName || 'Property Owner';

        if (!recipientEmail) {
          return NextResponse.json(
            { error: 'Please provide a "to" email for the seller.' },
            { status: 400 }
          );
        }
      ...
      recipientEmail = body.to || bid.vendor!.email!;
      recipientName = bid.vendor?.name || 'Contractor';
      propertyAddress = bid.deal.address;
      ...
      contractData = {
        ...
        terms: {
          ...
          depositAmount: bid.subtotal * 0.25,
      ...
      recipientEmail = body.to;
      recipientName = body.party2.name;
      propertyAddress = `${body.property.address}, ${body.property.city}, ${body.property.state} ${body.property.zip}`;
    ...
    return NextResponse.json(
      { error: 'Failed to send contract', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
