    ...
    const projects = deals.map((deal) => {
      ...
      if (deal.budgetLedger) {
        try {
          ...
          // Sum up all trades
          baselineTotal = Object.values(baseline).reduce((sum: number, val: any) => sum + (Number(val) || 0), 0) as number;
          actualsTotal = Object.values(actuals).reduce((sum: number, val: any) => sum + (Number(val) || 0), 0) as number;
          committedTotal = Object.values(committed).reduce((sum: number, val: any) => sum + (Number(val) || 0), 0) as number;
        } catch (e) {
