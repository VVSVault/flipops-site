...
### ScrollArea Component Styling
**Decision:** Custom scrollbar styling with explicit positioning overrides.
**Rationale:**
- Radix UI ScrollArea applies internal styles that need `!important` overrides
- Scrollbar width reduced to 6px (`w-1.5`) for cleaner appearance
- Explicit `top: 0, bottom: 0, right: 0` inline styles override Radix defaults
- Background set to transparent to prevent visual artifacts in dark mode
- Global CSS in `globals.css` ensures consistent styling across all ScrollAreas

### Card Component Padding Override
**Decision:** Use `py-0 gap-0` on Cards containing ScrollAreas.
**Rationale:**
- Default Card component has `py-6` and `gap-6` causing unwanted spacing
- ScrollAreas inside Cards need edge-to-edge scrollbar positioning
- Override at usage site rather than modifying base Card component

### Tab Styling (Underwriting Page)
**Decision:** Color-coded background tabs instead of border-based selection.
**Rationale:**
- Border-based tabs get clipped by Card's `rounded-xl` corners
- Each tab has distinct color identity (blue=Comps, amber=Repairs, emerald=Scenarios)
- Background highlight avoids overflow clipping issues
- Better visual hierarchy and accessibility

### NetSheet Summary Operator Alignment
**Decision:** Circular badge icons with explicit `mt-5` offset.
**Rationale:**
- Operators (−, =, →) need to align with number values, not column labels
- Using Lucide icons (`Minus`, `Equal`, `ChevronRight`) instead of text characters
- Circular gray/green backgrounds provide visual polish
- `mt-5` pushes icons down to align with number baseline

### Seed Comps for Demo Properties
**Decision:** Hardcoded realistic comps for specific demo properties.
**Rationale:**
- "1234 Oak Street" (prop-001) seeded with 6 Jacksonville-area comps
- Realistic price/sqft ratios ($179-$195) for the market
- 4 pre-selected, 2 available but unselected by default
- Enables full ARV calculation demo without API dependency