...
const OLD_WORKFLOW_ID = 'vBfMuxa74Wq73OGJ';

async function fixSplitBatchesWorkflow() {
  console.log('🗑️  Deleting simplified workflow...\n');
  ...
  try {
    ...
    console.log('📋 Creating Data Refresh workflow with FIXED Split Into Batches...\n');
...
console.log(\`Found \${deals.length} active deals to sync\`);
if (deals.length === 0) {
  return [{ json: { noDeal: true, message: 'No active deals to sync' } }];
}
...
return [{ json: message }];`,
      ...
      connections: {
        ...
        'Split Into Batches': {
          main: [
            [{ node: 'Refresh Deal', type: 'main', index: 0 }],
            [{ node: 'Track Results', type: 'main', index: 0 }],
          ],
        },
        'Refresh Deal': {
          main: [[{ node: 'Split Into Batches', type: 'main', index: 0 }]],
    ...
    console.log('   • Refreshes data in batches of 5 with LOOP-BACK connection');
    console.log('   • Tracks success/failure results');
    console.log('   • Alerts on sync failures via Slack');
    console.log('\n✅ Workflow with properly configured Split Into Batches!');
